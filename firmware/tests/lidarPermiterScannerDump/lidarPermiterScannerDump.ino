...
void loop()
{
  ...
  if(scanActive == true)
  {
    intervalReport = random(20, 40);//possible use to distribute data
    ...
    if(millis() - previousMillisReport > intervalReport) 
    {
      ...
      //try again
      if(panCurrentPosition < 0 || panCurrentPosition > PAN_MAX)
      {
        delay(10);     
        panCurrentPosition =   ax12GetRegister(1, AX_PRESENT_POSITION_L, 2);  //get current position from servo
...
//        Serial.write(255-(highByte(mappedPanCurrentPosition) + lowByte(mappedPanCurrentPosition) + highByte(distance) + lowByte(distance))%256);    
          int memoryMmappedPosition = floor(mappedPanCurrentPosition / 4);
          distanceArray[memoryMmappedPosition] = distance;
        ...
        if(panGoalPositon == PAN_MIN)
        {
          ...
          {  
            if(distanceArray[i] > -1)
            {
              Serial.write(0xff);//header1
              Serial.write(0xff);//header2
              Serial.write(highByte(i*4));//position high byte
              Serial.write(lowByte(i*4));//position low byte
              Serial.write(highByte(distanceArray[i]));//distance high byte
              Serial.write(lowByte(distanceArray[i]));//distance low byte
              Serial.write(255-(highByte(4*i) + lowByte(4*i) + highByte(distanceArray[i]) + lowByte(distanceArray[i]))%256);    
              delay(10);
            }
          }
